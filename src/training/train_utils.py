...
def save_prediction_plot(path: Path, y_true: np.ndarray, y_pred: np.ndarray, model_name: str) -> None:
...
def train_and_evaluate(
    *,
    ...
    best_loss = float("inf")
    best_epoch = 0
    output_dir = output_root / model_name / f"horizon_{args.horizon}" / f"seed_{args.seed}"
    ...
    save_prediction_plot(output_dir / "prediction_plot.png", y_true, y_pred, model_name)
